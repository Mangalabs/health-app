...
export function MedicationsCard() {
  ...
  const todayLogsMap = useMemo(() => {
    const map = new Map<string, { taken: boolean; statuses: string[] }>()
    medicationLogs.forEach((log) => {
      const date = toDateStr(log.loggedAt)
      if (date !== todayStr) return
      const key = log.medicationId
      if (!map.has(key)) {
        map.set(key, { taken: false, statuses: [] })
      }
    ...
    return map
  }, [medicationLogs, todayStr])

  const pendingMedications = useMemo(() => {
    return medications
      .filter((med) => med.active !== false)
      .filter((med) => {
        const logData = todayLogsMap.get(med.id)
        return !logData || !logData.taken
      })
  }, [medications, todayLogsMap])
  ...
  const getStatusBadge = (medicationId: string) => {
    const logData = todayLogsMap.get(medicationId)
  ...
  const isDone = (medicationId: string) => {
    const logData = todayLogsMap.get(medicationId)
    return logData?.taken || false
  }
