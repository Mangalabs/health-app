...
const INITIAL_HYDRATION_LOGS: HydrationLog[] = [
...
export const healthApi = {
  ...
  getWeightLogs: async (): Promise<WeightLog[]> => {
    await delay(300)
    const stored = await storage.getItem<WeightLog[]>(DB_KEYS.WEIGHT)
  ...
  addWeightLog: async (weightKg: number): Promise<void> => {
    ...
    // Pegamos os logs do storage ou inicializamos com os mocks
    const logs =
      (await storage.getItem<WeightLog[]>(DB_KEYS.WEIGHT)) ||
      INITIAL_WEIGHT_LOGS

    // CORREÇÃO: Procuramos se já existe um peso registrado hoje
    const existingIndex = logs.findIndex((log) => log.date === today)

    if (existingIndex >= 0) {
      // Se existir, nós ATUALIZAMOS o valor em vez de criar um duplicado
      logs[existingIndex].weightKg = weightKg
    } else {
